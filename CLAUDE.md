...
- **Setup is inline, not a skill** (`issue-workflow`) -- the orchestrator does the branch/work-dir/Issue.md work in bash, then hands a free-form prompt to `claude` for repo-specific environment setup. That session still owes the same completion record as every skill: the prompt instructs it to write `.session-result.json` (`stage: "setup"`) as its last step. Without one, a finished setup session is indistinguishable from one still installing, and an unattended run parks there
...
- **Human-only commands:** A skill marked `disable-model-invocation` cannot be invoked by a session -- `/code-review` is one, and an attempt at it silently degrades into nothing or into a weaker substitute wearing the same name. Skills must never simulate one, report it as done, or run a stand-in (`/simplify` and `/security-review` are each a subset of it). They hand off instead: the final message ends with `REVIEW PASS OUTSTANDING -- run: /code-review --fix`, and issue-workflow's `.session-result.json` carries `operator_request: {command, reason}`. The session deliberately stays open so the command can land in it, rather than exiting and losing the context
- **Nothing turn-ending before mandatory steps:** a slash-command sub-skill that finishes by reporting to the user ends the session's turn -- `/security-review` does. Placed mid-stage it strands the run: later steps never execute, no session result is written, and an unattended session idles forever (observed on #903, which sat ~1h20m until nudged). Sub-skills that return control (`/simplify`) are safe, but the ordering rule is the durable one
